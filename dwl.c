...
drawbar(Monitor *m)
{
	...
	int boxw = m->drw->font->height / 6 + m->drw->font->height / 2;
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		...
		tw = TEXTW(m, stext) - m->lrpad + m->drw->font->height / 2; /* nice right padding */
...
updatebar(Monitor *m)
{
	...
	m->b.height = m->drw->font->height + m->drw->font->height / 2;
